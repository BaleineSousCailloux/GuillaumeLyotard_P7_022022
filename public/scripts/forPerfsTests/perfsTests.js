////////// code nécessaire en commmun aux 2 algorithmes /////////////
const recipes = [
    {
        "id": 1,
...
///////// boucles natives  /////////
let saisieUtilisateur = "chocolat";
const isMatchingUserInput = (recette, saisieUtilisateur) => {
    let userInputTest = true;
    if (saisieUtilisateur.length >= 3) {
        let instanceSaisie = uniformise(saisieUtilisateur);
        ...
        do {
            ...
            for (let i = 0; i < recette.ingredients.length; i++) {
                let ingredientUniformise = uniformise(recette.ingredients[i].ingredient);
                if (ingredientUniformise.includes(saisieUser[n])) {
                    ingredientsTest = true;
                    i = recette.ingredients.length;
                }
            }
            const titreTest = uniformise(recette.nom).includes(saisieUser[n]);
            const descriptionTest = uniformise(recette.description).includes(saisieUser[n]);
...
do {
    ...
    const isMatching = isMatchingUserInput(recettesListe[z], saisieUtilisateur);
    isRecetteMatching = isRecetteMatching || isMatching;
    if (isRecetteMatching) {
        recettesTrouvees++;
    }
    z++;
} while (z < recettesListe.length);
console.log(recettesTrouvees);
...
const isMatchingUserInput = (recette, saisieUtilisateur) => {
    ...
    if (saisieUtilisateur.length >= 3) {
        ...
        saisieUser.forEach(motSaisi => {
            const ingredientsTest = recette.ingredients.find(ingredient => uniformise(ingredient.ingredient).includes(motSaisi));
            const titreTest = uniformise(recette.nom).includes(motSaisi);
            const descriptionTest = uniformise(recette.description).includes(motSaisi);
            userInputTest = userInputTest && (ingredientsTest || titreTest || descriptionTest);
        });
    }
    return userInputTest;
...
recettesListe.forEach(recette => {
    let isRecetteMatching = false;
    const isMatching = isMatchingUserInput(recette, saisieUtilisateur);
    isRecetteMatching = isRecetteMatching || isMatching;
    if (isRecetteMatching) {
        recettesTrouvees++;
    }
